...
Server must be running on http://localhost:7000 (or set BASE_URL env var).
"""
...
def test_ch02_post_config():
    ...
    check("Unknown fields silently ignored (extra key)", True)
...
def test_ch06_get_proxy():
    ...
    j = r2.json()
    required = {"id", "url", "status", "last_checked_at", "consecutive_failures",
...
def test_alert_lifecycle(webhook_url: str):
    ...
    good = [f"{BASE_URL}/health/px-good-00{i}" for i in range(1, 6)]
